            ...
            else if (_model is INGramModel ngramModel)
            {
                try
                {
                    if (_batchProvider is TokenBatchProvider provider)
                    {
                        ...
                        Console.WriteLine($"[B2] Навчено {ngramModel.ModelKind} на {allTokens.Length} токенах за {_config.Epochs} епох.");
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"[B2 Error] Помилка під час навчання NGram: {ex.Message}");
                    throw;
                }
            }
        }
